...
class PoseLandmarkerHelper(
    ...
    private val mStreamOutlet = LSL.StreamOutlet(
        LSL.StreamInfo(
            "Pose", "other", 33 * 3,
            LSL.IRREGULAR_RATE, LSL.ChannelFormat.float32, "localhost"
        )
    )
    ...
    private fun returnLivestreamResult(
        ...
        if(result.worldLandmarks().size>0) {
            for (idx in result.worldLandmarks().indices) {
                ...
                mStreamOutlet.push_sample(samples.toFloatArray())
            }
        }
